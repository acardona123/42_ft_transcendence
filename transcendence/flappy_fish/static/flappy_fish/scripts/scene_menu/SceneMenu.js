class SceneMenu extends Phaser.Scene{
	...
	create(){
		...
		#createCeiling(){
			this.#ceiling = new MenuCeiling(this.#scene_textures.ceiling);
		}
		...
		#createGround(){
			this.#ground = new MenuGround(this.#scene_textures.ground);
		}
		...
		#createPlayerPanels(){
			...
			#positionPayerPanels(){
				...
				#positionPayer1Panel(){
					...
					const y = gameConfig.scene_menu.ceiling.height + gameConfig.scene_menu.padding.top;
		...
		#createTimeLimit(){
			this.#match_duration = new EntitledTimeDisplay(this, "Time limit", gameMode.maxTime, gameConfig.scene_menu.text_style);
		...
		#createDeathsLimit(){
			const deaths_limit_content = `Deaths limit: ${(gameMode.maxDeath < 0) ? "∞" : gameMode.maxDeath}`;
			this.#deaths_limitation = this.add.text(0, 0, deaths_limit_content, gameConfig.scene_menu.text_style);
			this.#positionDeathsLimit();
		}
			#positionDeathsLimit(){
				this.#deaths_limitation.setOrigin(0.5, 0);
				const x = gameConfig.width / 2;
				const y = this.#match_duration.y + this.#match_duration.height / 2 + gameConfig.scene_menu.padding.under_time_limit;
