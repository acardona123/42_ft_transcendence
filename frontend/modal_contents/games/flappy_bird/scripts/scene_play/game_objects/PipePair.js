...
class fb_PipePair extends Phaser.GameObjects.Container{
	...
	constructor(
		...
		pipe_group,
		pipe_textures,
		spacer_height = fb_gameConfig.scenePlay.pipe_spacer.height_default,
		offset_to_middle = 0,
		x = fb_gameConfig.width + (Math.max (fb_gameConfig.scenePlay.pipe.core_width, fb_gameConfig.scenePlay.pipe.head_width, fb_gameConfig.scenePlay.pipe_spacer.width))/2
		){
		super(scene);
		this.#scene = scene;
		this.#pipe_group = pipe_group;
		this.depth = fb_gameConfig.scenePlay.depth.pipes;
	...
	#calculateScales(targeted_spacer_height){
		...
		#calculateScaleCore(){
			const flyable_zone_height = fb_gameConfig.height - fb_gameConfig.scenePlay.ground.height - fb_gameConfig.scenePlay.ceiling.height;
			this.#pipe_core_height = flyable_zone_height - fb_gameConfig.scenePlay.pipe_spacer.height_min - 2 * fb_gameConfig.scenePlay.pipe.head_height;
			this.scales.core = {
				x: fb_gameConfig.scenePlay.pipe.core_width / fb_gameTextures.pipe.core.width,
				y: this.#pipe_core_height / fb_gameTextures.pipe.core.height
			};
		}
		#calculateScaleHead(){
			this.scales.head = {
				x: fb_gameConfig.scenePlay.pipe.head_width / fb_gameTextures.pipe.head.width,
				y: fb_gameConfig.scenePlay.pipe.head_height / fb_gameTextures.pipe.head.height
			}
		}
		#calculateScaleSpacer(targeted_spacer_height){
			this.#spacer_height = clamp(targeted_spacer_height, fb_gameConfig.scenePlay.pipe_spacer.height_min, fb_gameConfig.scenePlay.pipe_spacer.height_max);
			this.scales.spacer = {
				x: fb_gameConfig.scenePlay.pipe_spacer.width / fb_gameTextures.pipe_spacer.width,
	...
	#positionComponents(){
		...
		#positionFirstComponent(container_components){
			...
			container_components[0].y = - (this.#pipe_core_height + fb_gameConfig.scenePlay.pipe.head_height + this.#spacer_height / 2);
		}
	...
	#integrateComponentsToContainer(){
		...
		#fitContainerToComponents(){
			...
			#fitContainerToComponentsWidth(){
				const container_width = Math.max (fb_gameConfig.scenePlay.pipe.core_width, fb_gameConfig.scenePlay.pipe.head_width, fb_gameConfig.scenePlay.pipe_spacer.width);
			...
			#fitContainerToComponentsHeight(){
				...
				const container_height = 2 * (this.#pipe_core_height + fb_gameConfig.scenePlay.pipe.head_height) + this.#spacer_height;
				this.setSize(container_width, container_height);
			}
	...
	reactivate(targeted_spacer_height = fb_gameConfig.scenePlay.pipe_spacer.height_default, offset_to_middle = 0, x = fb_gameConfig.width + this.width/2){
	...
	setVerticalOffset(offset_to_middle){
		offset_to_middle = clamp(offset_to_middle, -fb_gameConfig.scenePlay.pipe_repartition.vertical_offset_max, fb_gameConfig.scenePlay.pipe_repartition.vertical_offset_max);
