class Icon extends Phaser.GameObjects.Container {
	...
	constructor(player_index_symbol, scene_icon_texture, size = pg_gameConfig.scene_menu.player_panel.icon_size){
		...
		#createBackground(){
			...
			#getBackgroundColor(){
				if (this.#player_index_symbol === player_index.PLAYER1){
					return (pg_gameConfig.scene_play.player.color.left);
				} else {
					return (pg_gameConfig.scene_play.player.color.right);
				}
			}
	...
	tiltIcon(){
		let angle = pg_gameConfig.scene_menu.player_panel.icon_angle;
